...
export class Cell {
  ...
  static get Vertices() {
    return [
      ...
      new Vector2D(1, 0), // top-right corner
      new Vector2D(1, 1), // bottom-right corner
      new Vector2D(0, 1),  // bottom-left corner
      new Vector2D(0.5, 0.5), // center of the square
    ]
  }
...
export class Lattice {
   ...
   */
  allVertices() {
    let vertices = new Set<Vector2D>();

    for (let y = 0; y < this.height; y++) {
      for (let x = 0; x < this.width; x++) {
        for (const vertex of Cell.Vertices) {
          // Add the vertex to the set, offset by the cell coordinates
          vertices.add(vertex.add(new Vector2D(x, y)));
        }
  ...
  isPerimeterVertex(vertex: Vector2D) {
...
// import { isCloseTo, Vector2D, Cell, Lattice } from "./lattice"
...
//     test("returns all vertices of a 1x1 lattice", () => {
...
//     test("returns the right number of vertices for a 3x2 lattice", () => {
//       const lattice = new Lattice(3, 2);
//       const vertices = lattice.allVertices();
//       expect(vertices.size).toBe(18);  // worked it out by hand
//     })
//   })
...
//       const lattice = new Lattice(1, 1);
//       expect(lattice.isPerimeterVertex(new Vector2D(0, 0))).toBe(true);
//     })

//     test("returns false (1, 1) in a 2x2 lattice", () => {
//       const lattice = new Lattice(2, 2);
//       expect(lattice.isPerimeterVertex(new Vector2D(1, 1))).toBe(false);
//     })
//   })
// })
